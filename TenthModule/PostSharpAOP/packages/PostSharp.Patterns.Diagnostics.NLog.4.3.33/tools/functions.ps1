...
function SetProperty($psproj, [string] $propertyName, [string] $propertyValue, [string] $compareValue )
{
	...
	if (!$property -and !$compareValue )
	{
		...
		$property.SetAttribute("Value", $propertyValue)
      	...
      	$insertAfterNode = $xml.Project.Property | Select-Object -Last 1
        if (!$insertAfterNode)
        {
            $insertAfterNode = $xml.Project.License | Select-Object -Last 1
        }
	 	
        $xml.Project.InsertAfter($property, [System.Xml.XmlNode] $insertAfterNode)
	}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU+CeoxkVMVM5pxthY79BD/g4f
# 9/2ggg8NMIIE0zCCA7ugAwIBAgIQGNrRniZ96LtKIVjNzGs7SjANBgkqhkiG9w0B
...
# BDEWBBT8dgPc5JGn/v5i/vFWNNX6ieSQ4jANBgkqhkiG9w0BAQEFAASCAQCa+wE9
# Nveb4nRSST5XWTag8gbeSYmH+msQBkaBDKWscRBb28yC/qN62et2sJR/7Z+AD15d
# RxE6KLOVxIATFUHSfjU/VfK4XV1muvoinZXXpRsBrjv2fsW9xxEnoOq5pDAA0GUo
# OfW4IxFn/iTJcQI7rNOn80rtDcf0CPjZ4f7KsaMGgH9uHq4TgbjWPjNruZZiwM6P
# vOypezL483q52YCCAJ3JrXWFCq83MNrc3cOocPHlg5J3FRmE/79yDrIO0QOFafNc
# K3VVsrRVQbR2FSKnHGOnYuj/6i/4FAJbvJT5MLki5tagpP8euwbYiCH3+lA6ukZF
# J0MXYXLl3dkJi/EF
